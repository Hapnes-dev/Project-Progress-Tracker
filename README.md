...
- **Add / rename / remove a category**: **+ Add category** opens a dialog (name, **Shared/Private** type, optional **description** — like Rocketlane's Create-project-phase) and creates a matching Rocketlane **phase**, dated to today. **Right-click a category header** → *Rename / Remove* — renaming also renames the phase; removing (after a **double confirm**) deletes the phase and its tasks. Applies only to Rocketlane-linked projects (otherwise local-only).
