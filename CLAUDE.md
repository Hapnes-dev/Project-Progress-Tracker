...
- **Younium status modal showed "Invoiced" instead of Younium's real "Partially paid" order status** — the modal's "Order status" is `verdict.deliveryStatus || verdict.orderStatus`, where `deliveryStatus` comes from `youniumDeliveryStatusLabel(order.status)`. That helper only knew the delivery dimension (7 = Partially delivered, 8 = Delivered), so an invoiced-then-partially-paid order (Younium `status` **10**) returned null and fell through to the invoice-derived "Invoiced". Fix: the helper now also maps the **payment dimension** — **10 → "Partially paid"**, **11 → "Paid"** — and both render **green** (`orderIsGood`, `youniumRelatedOrderBadgeClass`), since an invoiced + paying order is a healthy end state, not a warning. Also added 10/11 to `YOUNIUM_STATUS_LABEL`. Verified live on O-014603 (`status:10` → "Partially paid", green ✓). Note: the `out.deliveryStatus` property name is now a slight misnomer (it carries delivery *or* payment header status) — kept as-is to avoid a risky rename; the `youniumApplyPartialDeliveryDowngrade` green→yellow downgrade still only fires on `=== "Partially delivered"`, so "Partially paid" is unaffected. 11 = "Paid" is inferred from the pair (no fully-paid order seen yet).
