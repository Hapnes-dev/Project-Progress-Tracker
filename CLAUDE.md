...
| Zendesk ticket comment cache — keyed by ticket `updated_at`, capped at 80 (v3: latest INCOMING reply + a `repliedByMe` flag) | `zendesk_ticket_cache_v3` |
...
- Per candidate: reuse the cached **latest incoming reply** (`entry.latestIncoming` = newest PUBLIC comment **not** authored by me) + an `entry.repliedByMe` flag (is the newest public comment mine?) when its `updated_at` is unchanged; otherwise fetch `/tickets/{id}/comments.json` and refresh the cache. **Only changed/new tickets hit the comments API** (verified: 14 comment calls cold, 0 on a no-change repeat). Author names resolved via one `/users/show_many.json` for ids not already cached.
...
- **Badge** calls it with `since = zendeskGetNotifLastSeen()` + applies the Hide-solved/closed filter to the count so it matches the list. **Drawer** (`notifZdRefresh`) calls it with `since = now − zdMonthsShown × ZENDESK_LIST_WINDOW_MS` (default 1 month; "Show more" bumps `zdMonthsShown` up to `ZD_MAX_MONTHS` = 6) and `{maxCandidates:300, maxPages:…}`, then renders with `newSinceMs = lastSeen` so replies newer than last-seen get a red **New** pill. Sorted by reply time desc. `Clear all` re-renders from `notifZdTicketsCache` without New flags (keeps the list visible).
